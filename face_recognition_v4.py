...
class FaceRecognizer_v4:
    def __init__(self, user_root="dataset-images/user-test", retrain=False):
        ...
        self.model_filename = "facebin-artifacts/facebin_model_v4.h5"
    ...
    def create_model(self):
        ...
        self.encoder = Model(input_img, vgg_descriptor)
        self.encoder.load_weights(self.vgg_model_filename, by_name=False)
        ...
        facebin = Dense(2048, activation='elu')(facebin)
        facebin = Dense(1024, activation='elu')(facebin)
        facebin = Dense(512, activation='elu')(facebin)
        facebin = Dense(512, activation='elu')(facebin)
        facebin = Dense(
    ...
    def train(self):
        ...
        tensor_board = TensorBoard(
            log_dir='/tmp/face_recognition_v4/tf-{}'.format(time.time()),
            histogram_freq=0,
        ...
        model_checkpoint = ModelCheckpoint(
            "/tmp/facebin-model-v4-val_acc:{val_acc:.2f}.h5",
            monitor='val_acc',
            verbose=1,
            save_best_only=True,
            period=1)
    ...
    def predict_faces(self, image):
        ...
        if len(faces) > 0:
            # log.debug("faces 0: %s", faces)
            faces = faces.astype(np.float32, casting='safe') / 255.0
            # log.debug("faces 1: %s", faces)
            # faces = preprocess_input(faces)
            # log.debug("faces 2: %s", faces)
    ...
    for d in glob.glob(
        ...
        for f in glob.glob(d + '/*.png'):
            ...
            try:
                ...
                results_dict[person_name] = results_dict[person_name] + 1 if (
                    person_name) in results_dict else 1
                if os.path.basename(d) == person_name:
                    num_correct += 1
                else:
                    num_incorrect += 1
